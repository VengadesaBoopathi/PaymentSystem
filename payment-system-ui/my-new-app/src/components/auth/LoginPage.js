  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!handleErrors(response.data.errors)) {
        ...
        if (
          !handleErrors(responseProfile.data.errors) &&
          !handleErrors(responseUser.data.errors)
        ) {
          sessionStorage.setItem(
            "profileType",
            responseProfile.data.object.profileType
          );
          ...
          if (responseProfile.data.object.profileType === "CUSTOMER") {
            const responseCustomer = await axios.get(
              `${CUSTOMER_URL}/${responseUser.data.object.email}`
            );
